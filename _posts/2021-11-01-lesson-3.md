...
Create a .txt file in the same directory as your code and copy/paste the [bee movie script](http://www.script-o-rama.com/movie_scripts/a1/bee-movie-script-transcript-seinfeld.html) into it. Then, write a program that prints it line by line by reading the text file.

Bonus: at the end, for each letter in the alphabet print how many times it occurs! One way you can do this is to use a dictionary.
...
Write a program that takes in two integers as input, as you have done previously. But this time, use `try` and `except` to print an error if the user tries to divide by 0.
