...
Similarly to lists, dictionaries are *iterables* meaning you can use for loops to iterate a dictionary. The variable after the word `for` in the loop will represent the key. The below code presents an example.
...
If you think you've got a good grasp on dictionaries and lists, try solving the following problem:
